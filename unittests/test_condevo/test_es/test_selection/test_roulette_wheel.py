...
class TestEsUtils(TestCase):
    ...
    def test_roulette_wheel(self):
        ...
        f = roulette_wheel(np.log(x), consider_nans=False)
        ...
        f1 = roulette_wheel(np.log(x), consider_nans=True)
    ...
    def test_basic_functionality_tensor_positive_s(self):
        ...
        result = roulette_wheel(f, s=3.0, normalize=True)
    ...
    def test_basic_functionality_numpy_negative_s(self):
        ...
        result = roulette_wheel(f, s=-3.0, normalize=True) # Negative s favors lower fitness

        self.assertTrue(np.isclose(result.sum(), np.array(1.0)))
        # Expect higher values to have higher probabilities with negative s, but with log slope instead of exp
        self.assertGreater(result[4], result[3])
        self.assertGreater(result[3], result[2])
        self.assertGreater(result[2], result[1])
        self.assertGreater(result[1], result[0])
    ...
    def test_basic_functionality_tensor_no_normalize(self):
        ...
        result = roulette_wheel(f, s=1.0, normalize=False)
    ...
    def test_single_element_tensor(self):
        ...
        result = roulette_wheel(f, normalize=True)
        self.assertTrue(torch.isclose(result.sum(), torch.tensor([1.0])))
        result_no_norm = roulette_wheel(f, normalize=False)
    ...
    def test_all_same_fitness(self):
        ...
        result = roulette_wheel(f, s=5.0, normalize=True)
    ...
    def test_s_equals_zero(self):
        f = torch.tensor([1.0, 2.0, 3.0], dtype=torch.float32)
        result = roulette_wheel(f, s=0.0, normalize=True)
        # print(result)
        self.assertGreater(result[2], result[1])
        self.assertGreater(result[1], result[0])
    ...
    def test_eps_impact(self):
        f = torch.tensor([1.0, 2.0, 3.0], dtype=torch.float32)
        result_default_eps = roulette_wheel(f, s=3.0, normalize=True)
        result_larger_eps = roulette_wheel(f, s=3.0, normalize=True, eps=1e-1)
        self.assertFalse(torch.allclose(result_default_eps, result_larger_eps))

        f_same = torch.tensor([1.0, 1.0], dtype=torch.float32)
        result_same = roulette_wheel(f_same, eps=1e-12)
    ...
    def test_consider_nans_false(self):
        ...
        result = roulette_wheel(f, consider_nans=False, normalize=True)
    ...
    def test_consider_nans_true(self):
        ...
        f = torch.tensor([1.0, float('nan'), 3.0, float('inf'), 5.0], dtype=torch.float32)
        result = roulette_wheel(f, consider_nans=True, normalize=True)
    ...
    def test_threshold_positive(self):
        ...
        result = roulette_wheel(f, threshold=0.5, normalize=True)
    ...
    def test_threshold_zero(self):
        f = torch.tensor([1.0, 2.0, 3.0], dtype=torch.float32)
        result_no_threshold = roulette_wheel(f, threshold=0, normalize=True)
        result_with_zero_threshold = roulette_wheel(f, threshold=0.0, normalize=True)
        self.assertTrue(torch.isclose(result_no_threshold, result_with_zero_threshold).all())

    def test_threshold_max_value(self):
        f = torch.tensor([1.0, 2.0, 3.0, 10.0], dtype=torch.float32)
        result = roulette_wheel(f, threshold=1.0, normalize=True)
    ...
    def test_numpy_input_with_nans_and_threshold(self):
        ...
        result = roulette_wheel(f, s=1.0, normalize=True, consider_nans=True, threshold=0.5)
    ...
    def test_all_nans_or_infs(self):
        ...
        result_no_nans = roulette_wheel(f, consider_nans=False)
        ...
        result_with_nans = roulette_wheel(f, consider_nans=True, normalize=True)
